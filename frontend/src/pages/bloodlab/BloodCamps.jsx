...
const BloodCamps = () => {
  ...
  const fetchCamps = useCallback(async (page = 1) => {
    try {
      setLoading(true);
      ...
      const url = `/blood-lab/camps?${queryParams}`;
  ...
  const updateCampStatus = async (campId, newStatus) => {
    try {
      ...
      const endpoints = [
        `/blood-lab/camps/${campId}/status`,
        `/blood-lab/camps/${campId}`,
        `/blood-lab/camps/${campId}/status`,
      ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Try different endpoint variations
      const baseUrls = [`/blood-lab/camps`, `/blood-lab/camps`];
  ...
  const handleDeleteCamp = async (id) => {
    ...
    try {
      // Try different endpoint variations
      const endpoints = [`/blood-lab/camps/${id}`, `/blood-lab/camps/${id}`];
